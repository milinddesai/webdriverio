...
export const config: WebdriverIO.Config = {
    ...
    onComplete: function (exitCode, config, capabilities, results) {
        return new Promise<void>((resolve, reject) => {
            ...
            const allureReportDir = './reports/allure-report';


            try {
                // Generate the Allure report using allure-commandline programmatically
                const generation = allure(['generate', allureResultsDir, '--clean', '-o', allureReportDir]);

                generation.on('exit', (code: number) => {
                    if (code === 0) {
                        console.log(`Allure report generated successfully in ${allureReportDir}`);

                        // Conditionally open report based on environment variable
                        if (process.env.OPEN_ALLURE_REPORT === 'true') {
                            console.log('Opening Allure report in browser...');
                            const openProcess = allure(['open', allureReportDir]);

                            openProcess.on('exit', (openCode: any) => {
                                if (openCode === 0) {
                            ...
                            console.log(`To view the report, run: allure open ${allureReportDir}`);
                            resolve();
                        }
                    } else {
                ...
                console.error('Error starting Allure report generation:', error.message);
                // Don't fail the test run if report generation fails
                console.error('Test results are still valid despite report generation failure.');
                resolve(); // Resolve instead of reject to not fail the test run
            }
        });
    },
    /**
